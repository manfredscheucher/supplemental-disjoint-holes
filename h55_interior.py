...
"""
	...
	to verify that every set of 14 points contains
	two interior-disjoint 5-holes.
...
n = int(argv[1]) # run program with command line parameter 15
N = range(n)
...
all_variables += [('is5hole',I) for I in combinations(N,5)]
all_variables += [('hole_left',I) for I in permutations(N,2)]
all_variables += [('hole_right',I) for I in permutations(N,2)]
...
def var_is5hole(*L): return var(('is5hole',L)) 
def var_hole_left (*L): return var(('hole_left' ,L)) 
def var_hole_right(*L): return var(('hole_right',L)) 
...
print "(8*) Forbid interior-disjoint 5-holes",len(constraints)
for a,b in permutations(N,2):
	for I in combinations(N,5):
		if a in I:
			constraints.append(
				 [-var_is5hole(*I)]
				+[-var_trip(a,b,c) for c in I if c != a and c != b]
				+[var_hole_left(a,b)]
				)
	...
	constraints.append([-var_hole_left(a,b),-var_hole_right(a,b)])



print "(9) Harborth's result",len(constraints)

for k in range(10,n):
	constraints.append([var_is5hole(*I) for I in combinations(range(k-10,k),5)]) # harborth

for I in combinations(range( 0, n-10),5): # no 5-hole in {0..6}
	constraints.append([-var_is5hole(*I)]) # otherwise harborth gives 5 hole in {7..16}

for I in combinations(range(10, n),5): # no 5-hole in {10..16}
	constraints.append([-var_is5hole(*I)]) # otherwise harborth gives 5 hole in {0..9}
...
fp = "instance_h55_"+str(n)+"_interior.in"
f = open(fp,"w")
